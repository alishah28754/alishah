...
/// Shows "{n} sold" plus a colored stock badge — green "In Stock" when units
/// remain, red "Out of Stock" once sold count reaches the stock count.
/// Pass raw `soldLabel` (e.g. "120" or "120 sold") and the product's `stock`.
class SoldStockRow extends StatelessWidget {
  ...
  bool get _hasSoldInfo => (soldLabel ?? '').trim().isNotEmpty && !_isOutOfStock;
  bool get _hasStockInfo => stock > 0;
  bool get _isOutOfStock => _hasStockInfo && _soldCount >= stock;

  @override
  Widget build(BuildContext context) {
    if (!_hasSoldInfo && !_hasStockInfo) return const SizedBox.shrink();
    ...
    return Row(
      ...
      children: [
        ...
        if (_hasSoldInfo && _hasStockInfo) const SizedBox(width: 5),
        if (_hasStockInfo)
          Flexible(
            child: Text(
              ...
              style: TextStyle(
                ...
                fontFamily: kFont,
                color: _isOutOfStock ? AppColors.saleRed : const Color(0xFF1E8E3E),
              ),
            ),
          ),
      ],
