...
class _HomePageState extends State<HomePage> with AutomaticKeepAliveClientMixin {
  ...
  List<Product> _premiumCollection = const [
    Product(
      id: 'prem-001',
  ...
  List<NewArrivalProduct> _newArrivals = const [
    NewArrivalProduct(
      id: 'new-001',
  ...
  List<FlashSaleProduct> _flashSale = const [
    FlashSaleProduct(
      id: 'flash-001',
  ...
  List<ForYouProduct> _forYou = const [
    ForYouProduct(
      id: 'foryou-001',
  ...
  List<Product> get _allProducts {
    ...
    // Add New Arrivals
    all.addAll(_newArrivals.map((p) => p.toProduct()));

    // Add Flash Sale
    all.addAll(_flashSale.map((p) => p.toProduct()));

    // Add For You
    all.addAll(_forYou.map((p) => p.toProduct()));

    return all;
  }

  // ============================================================
  // ✅ SEARCH FILTER - Case-insensitive, CONTAINS (anywhere in name)
  // ============================================================
  List<Product> get _filteredProducts {
    ...
    return _allProducts.where((product) {
      ...
      // ✅ Matches anywhere in the product name
  ...
  // ============================================================
  // ✅ LIVE CATALOG - loads banners/products from the KTEX backend.
  // If the API is unreachable or returns nothing, the widget keeps the
  // built-in fallback data so the home screen never looks empty.
  // ============================================================
  Future<void> _loadCatalogData() async {
    ...
    try {
      final arrivals = await api.fetchNewArrivals();
      if (arrivals.isNotEmpty && mounted) {
        setState(() {
          _newArrivals = arrivals
              .map((p) => NewArrivalProduct(
                    id: p.id,
                    name: p.name,
                    imageUrl: p.imageUrl,
                    price: p.price,
    ...
    try {
      final flash = await api.fetchFlashSale();
      if (flash.isNotEmpty && mounted) {
        setState(() {
          _flashSale = flash
              .map((p) => FlashSaleProduct(
                    id: p.id,
                    name: p.name,
                    imageUrl: p.imageUrl,
                    price: p.price,
    ...
    // Only load "For You" and Premium when a browse/search is not active,
    // and prefer backend data when it exists.
    try {
      final forYou = await api.fetchForYou();
      if (forYou.isNotEmpty && mounted) {
        setState(() {
          _forYou = forYou
              .map((p) => ForYouProduct(
                    id: p.id,
    ...
    try {
      final premium = await api.fetchProducts(isPremium: true, limit: 10);
      if (premium.isNotEmpty && mounted) {
        setState(() => _premiumCollection = premium);
      }
    } catch (_) {}
  }
  ...
  // ============================================================
  // ✅ UPDATED: STICKY TOP BAR WITH SEARCH
  // ============================================================
      ...
      title: Padding(
        ...
        child: Row(
          children: [
            ...
            // ✅ UPDATED: Search Bar with Live Filtering
            Expanded(
              child: Container(
                height: 40,
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: isDark ? Colors.white.withOpacity(0.1) : Colors.black.withOpacity(0.3),
                  ),
                ),
                child: TextField(
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isDark ? Colors.white.withOpacity(0.1) : AppColors.gold.withOpacity(0.2),
          ),
          boxShadow: [
            BoxShadow(
  ...
  // ============================================================
  // FOR YOU SECTION (only show when NOT searching)
  // ============================================================
