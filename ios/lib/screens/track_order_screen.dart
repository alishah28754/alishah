...
class _TrackOrderScreenState extends State<TrackOrderScreen> {
  ...
  Future<void> _trackOrder() async {
    ...
    // 1) Live lookup from the backend (public tracking endpoint).
    Order? foundOrder = await _orderModel.trackViaApi(orderId);

    // 2) Fallback: search the local OrderModel (orders placed offline).
    if (foundOrder == null) {
